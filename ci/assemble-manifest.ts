...
/**
 * ci/assemble-manifest.ts — combine the emit skeleton with the cosign-produced
 * sigstore bundles into the final `report-manifest.json` the dashboard fetches
 * (bead nr75.12).
 *
 * Lives in `ci/` (NOT the published `scripts/`) so the zero-dep package surface
 * is untouched. Runs AFTER `ci/emit-evidence.ts` and AFTER CI has signed each
 * `bundle-<i>.json` with `cosign sign-blob --bundle bundle-<i>.sigstore.json`.
 * Fail-closed: a missing bundle/sigstore file or a structural mismatch aborts
 * rather than publishing a half-built manifest.
 *
 * Usage:
 *   node --experimental-strip-types ci/assemble-manifest.ts [--dir build/evidence] [--out build/evidence/report-manifest.json]
 */
...
export function assemble(dir: string): ReportManifest {
  ...
  const rows: ManifestRow[] = skeleton.rows.map((row) => {
    ...
    if (!existsSync(sigPath)) {
      throw new Error(`missing sigstore bundle ${sigPath} — was cosign sign-blob run for ${row.bundleFile}?`);
    }
    return {
  ...
  if (!isReportManifestShape(manifest)) {
    throw new Error('assembled manifest failed the report-manifest shape check (would be rejected at ingest)');
  }
  return manifest;
}
...
if (invokedDirectly) {
  try {
    ...
    console.error('assemble-manifest FAILED (fail-closed):', err instanceof Error ? err.message : String(err));
    process.exit(1);
  }
}
