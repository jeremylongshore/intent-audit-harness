...
- **`report-lineage --report PATH`** (`scripts/report-lineage.py`) → one `gate-result/v1` row with canonical gate ID `audit-harness:ci:report-lineage`. Validates J-Rig `unified-report/v1` and `suite-report/v1` projections, checks Run/Grade identity, per-cell arithmetic, and unique `sample_index` slots within each sampling cell, and optionally proves a suite's Run set against its `eval-suite/v1` audit manifest. Clean rows carry the selected Grader identity, deterministic Run counts, report schema, and sample-balance metadata consumed by the rollout action's optional exact-byte binding. stdlib-only, offline, read-only, advisory-first; `--strict` turns unverifiable lineage into FAIL. It never opens SQLite or imports J-Rig, so the published harness remains zero-runtime-dependency. It reports sample-slot integrity but does not invent or verify `target_n` unless a future report contract declares that field.
...
one arrived, read it if so, and otherwise proceed on CI. The deterministic merge gate is this repo's own CI (`ci.yml`: self-check on Node 18/20/22, shellcheck, ruff, `python -m py_compile`, version-canonical-check, wrapper-byte-sync, gen-layer-applicability check, fp-rate) plus the separate `codeql. That was
...
repo the reviewer should be pointed at this repo's gate semantics — the universal `no-gate-weakening` rule, escape-scan REFUSE/CHALLENGE/FLAG classification, threshold-lowering attempts, and hand-edits to hash-pinned policy files (`.harness-hash`).
