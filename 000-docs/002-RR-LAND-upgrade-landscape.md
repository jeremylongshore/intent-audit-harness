...
| **AVID** (AI Vulnerability Database) | Open knowledge base | AI-failure taxonomy | Separation of **what failed (DB)** from **how it's classified (taxonomy)** is the right shape for cataloging escape-scan REFUSE patterns at scale. Borrow the taxonomy-vs-evidence split for the failure-shape catalog the matcher-map work will need. | [avidml.org](https://avidml.org/) |
...
| **Failure-shape taxonomy / catalog** | NO | YES — AVID-style separation of evidence + taxonomy | Future (matcher-map adjacent) |
